...
Once you have picked one, the node wears it: the button reads the adapter's name, and the adapter's picture is drawn on the node itself, so a graph holding four loaders is readable at a glance instead of being four identical buttons. It is the same picture the grid drew, by the same rule — the model's own icon, or the face of the character or set it is attached to — but not the generated initials mark: an adapter with neither an icon nor an attachment keeps its name on the button and draws nothing. A saved workflow stores only the digest, so the name and the picture are looked up from the shelf when the node loads; an unreachable server or a missing token leaves the start of the digest on the button and no picture, and nothing else breaks. The checkpoint, VAE and text-encoder loaders do the same, though a checkpoint, VAE or text encoder is rarely attached to anyone, so in practice it is the ones carrying their own icon that show a picture.
