...
``LoraLoader``.  The ``lora_path`` input is a plain STRING, so it also accepts
a path from any other pack.
...
class PixlStashApplyAdapter:
    ...
    def apply_adapter(
        ...
        if not path:
            raise RuntimeError(
                "PixlStash Apply Adapter: lora_path is empty. "
                "Wire it from a PixlStash Adapter Loader."
            )
