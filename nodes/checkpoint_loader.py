...
def local_copy(record: dict, *, label: str) -> str:
    ...
    if path is None:
        raise RuntimeError(
            ...
            f"{record.get('display_name') or record.get('filename') or record.get('id')}"
...
class PixlStashCheckpointLoader:
    ...
    def load_checkpoint(self, checkpoint_id: str):
        ...
        outputs = load_file(local_copy(record, label=LABEL))
    ...
    @staticmethod
    def _fetch_record(checkpoint_id: str, *, label: str = LABEL) -> dict:
